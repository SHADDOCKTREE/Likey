window.app = {
	 ...
	 * netty服务后端发布的url地址
	 */
	nettyServerUrl: 'ws://42.192.1.53:8088/ws',
	// nettyServerUrl: 'ws://192.168.73.1:8088/ws',
	//ws://192.168.0.100:8088/ws
	
	/**
	 * 后端服务发布的url地址
	 */
	serverUrl: 'http://42.192.1.53/likey_v1',
	// serverUrl: 'http://192.168.73.1:8080',
	...
	setTopicList: function(topicList) {
		var topicListStr = JSON.stringify(topicList);
		plus.storage.setItem("topicList", topicListStr);
	},
	getTopicList: function(){
		var topicListStr = plus.storage.getItem("topicList");
		if (!this.isNotNull(topicListStr)){
			return [];
		}
		return JSON.parse(topicListStr);
	},
	//保存评论列表
	setCommentList: function(commentList){
		var commentListStr = JSON.stringify(commentList);
		plus.storage.setItem("commentList",commentListStr);
	},
	//获取评论列表
	getCommentList: function(){
		var commentListStr = plus.storage.getItem("commentList");
		if (!this.isNotNull(commentListStr)){
			return [];
		}
		return JSON.parse(commentListStr);
	},
